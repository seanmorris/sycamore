...
require.register("codemirror/lib/codemirror.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "codemirror");
  (function() {
...
require.register("codemirror/mode/css/css.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "codemirror");
  (function() {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
// Distributed under an MIT license: https://codemirror.net/LICENSE
...
require.register("codemirror/mode/htmlmixed/htmlmixed.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "codemirror");
  (function() {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
// Distributed under an MIT license: https://codemirror.net/LICENSE
...
require.register("codemirror/mode/javascript/javascript.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "codemirror");
  (function() {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
// Distributed under an MIT license: https://codemirror.net/LICENSE
...
require.register("codemirror/mode/xml/xml.js", function(exports, require, module) {
  require = __makeRelativeRequire(require, {}, "codemirror");
  (function() {
    // CodeMirror, copyright (c) by Marijn Haverbeke and others
// Distributed under an MIT license: https://codemirror.net/LICENSE
...
var _Sycamore = require("./Sycamore");
...
var _MessageLinkView = require("./MessageLinkView");

var _MessageImageView = require("./MessageImageView");

var _MessageAudioView = require("./MessageAudioView");

var _MessageVideoView = require("./MessageVideoView");

var _MessageYoutubeView = require("./MessageYoutubeView");
...
var _EventDatabase = require("./matrix/EventDatabase");
...
var FeedView = /*#__PURE__*/function (_View) {
  ...
  function FeedView(args) {
    ...
    _defineProperty(_assertThisInitialized(_this), "profiles", new Map());

    _defineProperty(_assertThisInitialized(_this), "postSet", new Set());

    _this.args.posts = _this.args.posts || [];
    _this.args.room_id = _this.args.room_id || '!KaJxaqzQsDrINmbMht:matrix.org';
    _this.selector = IDBKeyRange.bound([_this.args.room_id, 'm.room.message', 0], [_this.args.room_id, 'm.room.message', Infinity]);
    _this.args.showForm = false;

    if (_this.args.room_id) {
      Promise.all([_EventDatabase.EventDatabase.open('events', 1), matrix.getToken()]).then(function (_ref) {
        var _ref2 = _slicedToArray(_ref, 2),
            database = _ref2[0],
            token = _ref2[1];

        if (_this.args.room_id !== '!KaJxaqzQsDrINmbMht:matrix.org') {
          _this.listen(matrix, 'login', function () {
        ...
        _this.syncHistory(_this.args.room_id);

        database.addEventListener('write', function (dbEvent) {
          if (dbEvent.detail.subType !== 'insert') {
            return;
          }

          var event = dbEvent.detail.record;

          if (event.room_id === undefined) {
            return;
          }

          var eventKey = [event.room_id, event.type, event.received];

          if (!_this.selector.includes(eventKey)) {
            return;
          }

          var messageView = _this.getEventView(event);

          _this.args.posts.push(messageView);
        });
        var controller = matrix.listenForRoomEvents(_this.args.room_id);

        _this.onRemove(function () {
          return controller.cancelled = true;
        });

        _this.listeners.set(_this.args.room_id, controller);
      });
    }
  ...
  _createClass(FeedView, [{
    key: "syncHistory",
    value: function syncHistory(roomId) {
      _EventDatabase.EventDatabase.open('events', 1).then(function (database) {
        matrix.syncRoomHistory(roomId, '', function (event) {
          var store = 'events';
          var index = 'event_id';
          var range = event.event_id;
          var type = _EventModel.EventModel;
          database.select({
            store: store,
            index: index,
            range: range,
            ...
            if (res.index) {
              ...
              database.insert('events', _EventModel.EventModel.from(event));
            }
          });
        });
      });
    ...
    value: function afterLogin() {
      ...
      var getToken = matrix.getToken();
      var getState = matrix.getRoomState(this.args.room_id);
      Promise.all([getToken, getState]).then(function (_ref3) {
        var _ref4 = _slicedToArray(_ref3, 2),
            token = _ref4[0],
            response = _ref4[1];

        response.filter(function (event) {
          return event.type === 'm.room.power_levels';
        }).forEach(function (event) {
          if (token.user_id in event.content.users) {
            _this2.args.showForm = true;
          }
        });
      });
    }
    ...
    key: "loadFeeds",
    value: function loadFeeds(feedUrl) {
      var _this3 = this;

      _EventDatabase.EventDatabase.open('events', 1).then(function (database) {
        var range = _this3.selector;
        var index = 'room_id+type+time';
        var store = 'events';
        var limit = false;
        var direction = 'next';
        var posts = [];
        database.select({
          ...
          var user_id = event.sender || event.user_id;

          var messageView = _this3.getEventView(event);

          _this3.getProfile(user_id).then(function (profile) {
          ...
          var _this3$args$posts;

          (_this3$args$posts = _this3.args.posts).push.apply(_this3$args$posts, posts);
        });
      });
    }
  }, {
    key: "getEventView",
    value: function getEventView(event) {
      var messageView;
      var user_id = event.sender || event.user_id;

      switch (event.content.msgtype) {
        case 'm.audio':
          messageView = new _MessageAudioView.MessageAudioView({
            issued: event.origin_server_ts / 1000,
            body: event.content.body,
            header: _Bindable.Bindable.make({
              author: user_id
            }),
            avatar: '/avatar.jpg',
          ...
          matrix.getMedia(event.content.url).then(function (localUrl) {
            messageView.args.url = localUrl;
          });
          break;

        case 'm.video':
          messageView = new _MessageVideoView.MessageVideoView({
            issued: event.origin_server_ts / 1000,
            body: event.content.body,
            header: _Bindable.Bindable.make({
              author: user_id
        ...
        case 'm.image':
          messageView = new _MessageImageView.MessageImageView({
            issued: event.origin_server_ts / 1000,
            body: event.content.body,
            header: _Bindable.Bindable.make({
              author: user_id
        ...
        default:
          messageView = new _MessageView.MessageView({
            issued: event.origin_server_ts / 1000,
            body: event.content.body,
            header: _Bindable.Bindable.make({
              author: user_id !== null && user_id !== void 0 ? user_id : null
            }),
            avatar: '/avatar.jpg',
            eventId: event.event_id,
            source: JSON.stringify(event.content, null, 4),
      ...
      if (event.content.sycamore && event.content.sycamore["public"] && !this.listeners.has(event.content.sycamore["public"])) {
        this.syncHistory(event.content.sycamore["public"]);
        var controller = matrix.listenForRoomEvents(event.content.sycamore["public"]);
        this.onRemove(function () {
          return controller.cancelled = true;
        });
    ...
    value: function loadFeed(feed) {
      ...
      fetch('/' + feed).then(function (response) {
        ...
        var _iterator = _createForOfIteratorHelper(messageLines),
            _step;

        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var messageLine = _step.value;
    ...
    value: function getProfile(userId) {
      ...
      if (this.profiles.has(userId)) {
        ...
        var profile = {};
        getProfile = matrix.getUserProfile(userId).then(function (response) {
      ...
      return getProfile.then(function (profile) {
        ...
        if (profile.avatar_url) {
          var getMedia = matrix.getMedia(profile.avatar_url);
          return getMedia.then(function (blobUrl) {
            profile.avatar_url_local = blobUrl;

            _this5.profiles.set(userId, profile);
    ...
    value: function loadMessage(messageUrl) {
      ...
      fetch('/messages/' + messageUrl + '.smsg').then(function (response) {
        ...
        return _this6.displayPost(message);
      });
    }
  }, {
    key: "displayPost",
    value: function displayPost(message) {
      ...
      var mime = message.header.mime.split(/[\/\s]/);
      var type = mime[0] === 'text' ? message.name.split('.').pop() : mime[0];

      switch (type) {
        case 'image':
          this.args.posts.push(new _MessageImageView.MessageImageView(message));
          break;

        case 'video':
          this.args.posts.push(new _MessageVideoView.MessageVideoView(message));
          break;

        case 'audio':
          this.args.posts.push(new _MessageAudioView.MessageAudioView(message));
          break;

        case 'url':
          var url = new URL(message.body);
          ...
          switch (url.host) {
            ...
            case 'www.youtube.com':
              ...
              this.args.posts.push(new _MessageYoutubeView.MessageYoutubeView(message));
              break;

            default:
              this.args.posts.push(new _MessageLinkView.MessageLinkView(message));
              break;
          }
        ...
        default:
          this.args.posts.push(new _MessageView.MessageView(message));
          break;
      }
    ...
    value: function createPost(event) {
      ...
      _Sycamore.Sycamore.getSettings().then(function (settings) {
        var message = {
          msgtype: 'm.text',
          body: _this7.args.inputPost,
          sycamore: {
        ...
        return matrix.putEvent(_this7.args.room_id, 'm.room.message', message);
      }).then(function () {
        _this7.args.inputPost = '';
      });
    }
  }, {
    key: "fileDropped",
    value: function fileDropped(event) {
      var _this8 = this;

      event.preventDefault();
      event.stopPropagation();

      var _iterator2 = _createForOfIteratorHelper(event.dataTransfer.items),
          _step2;
      ...
      try {
        var _loop = function _loop() {
          ...
          var baseType = file.type.split('/')[0];

          switch (baseType) {
            case 'image':
            case 'video':
            case 'audio':
              matrix.postMedia(file, file.name).then(function (response) {
                matrix.putEvent(_this8.args.room_id, 'm.room.message', {
                  msgtype: 'm.' + baseType,
                  body: file.name,
    ...
    key: "fileDragged",
    value: function fileDragged(event) {
      event.preventDefault();
      event.stopPropagation();
    }
  }]);
...
;require.register("Matrix.js", function(exports, require, module) {
"use strict";
...
var Matrix = /*#__PURE__*/function (_Mixin$with) {
  ...
  _createClass(Matrix, [{
    ...
    value: function getMedia(mxcUrl) {
      ...
      var getUrl = fetch("".concat(this.mediaUrl, "/download/").concat(url.pathname.substr(2))).then(function (response) {
    ...
    value: function streamServerEvents(chunkList, room_id, controller) {
      ...
      chunkList.chunk && chunkList.chunk.forEach(function (event) {
        var detail = new _EventModel.EventModel();
...
;require.register("MessageAudioView.js", function(exports, require, module) {
"use strict";
...
exports.MessageAudioView = void 0;
...
var MessageAudioView = /*#__PURE__*/function (_MessageView) {
  _inherits(MessageAudioView, _MessageView);

  var _super = _createSuper(MessageAudioView);

  function MessageAudioView() {
    var _this;

    _classCallCheck(this, MessageAudioView);
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message-audio.html'));

    return _this;
  }

  return MessageAudioView;
}(_MessageView2.MessageView);

exports.MessageAudioView = MessageAudioView;
});
...
;require.register("MessageImageView.js", function(exports, require, module) {
"use strict";
...
exports.MessageImageView = void 0;
...
var MessageImageView = /*#__PURE__*/function (_MessageView) {
  _inherits(MessageImageView, _MessageView);

  var _super = _createSuper(MessageImageView);

  function MessageImageView() {
    var _this;

    _classCallCheck(this, MessageImageView);
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message-image.html'));
...
exports.MessageImageView = MessageImageView;
});

;require.register("MessageLinkView.js", function(exports, require, module) {
"use strict";
...
exports.MessageLinkView = void 0;
...
var MessageLinkView = /*#__PURE__*/function (_MessageView) {
  _inherits(MessageLinkView, _MessageView);

  var _super = _createSuper(MessageLinkView);

  function MessageLinkView() {
    var _this;

    _classCallCheck(this, MessageLinkView);
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message-link.html'));
...
exports.MessageLinkView = MessageLinkView;
});
  ...
  _createClass(MessageModel, [{
    ...
    value: function verify() {
      ...
      return fetch(this.header.key).then(function (r) {
        ...
        var uri = 'data:text/plain;base64,' + keyText;
    ...
    value: function parseBytes(buffer) {
      ...
      return headBlob.text().then(function (headerText) {
        ...
        var bodyBlob = new Blob([bodySlice], {
          type: header.mime || 'text/plain; charset=utf-8'
        });
        ...
        var signatureBlob = new Blob([signatureData], {
          type: 'text/plain; charset=utf-8'
        });
        var signatureBlob2 = new Blob([signatureSlice], {
          type: 'text/plain; charset=utf-8'
        });
        return signatureBlob.text().then(function (signature) {
          return fetch('data:application/sycamore;base64,' + signature).then(function (response) {
            return response.arrayBuffer();
            ...
            var body = header.mime.substr(0, 4) === 'text' ? bodyBlob.text() : URL.createObjectURL(bodyBlob);
            var results = [header, body, signatureBlob2.text(), segments];
...
;require.register("MessageVideoView.js", function(exports, require, module) {
"use strict";
...
exports.MessageVideoView = void 0;
...
var MessageVideoView = /*#__PURE__*/function (_MessageView) {
  _inherits(MessageVideoView, _MessageView);

  var _super = _createSuper(MessageVideoView);

  function MessageVideoView() {
    var _this;

    _classCallCheck(this, MessageVideoView);
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message-video.html'));

    return _this;
  }

  return MessageVideoView;
}(_MessageView2.MessageView);

exports.MessageVideoView = MessageVideoView;
});

;require.register("MessageView.js", function(exports, require, module) {
"use strict";
...
var MessageView = /*#__PURE__*/function (_View) {
  ...
  function MessageView() {
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message.html'));

    _this.args.comments = [];
    _this.args.likes = 0;
    _this.likers = new Set();
    _this.args.showComments = true;
    var messageId = _this.args.event.event_id;
    _this.selector = IDBKeyRange.bound([messageId, 0], [messageId, Infinity]);
    Promise.all([_EventDatabase.EventDatabase.open('events', 1), matrix.getToken()]).then(function (_ref) {
      var _ref2 = _slicedToArray(_ref, 2),
          database = _ref2[0],
          token = _ref2[1];

      var store = 'events'; // const index = 'replyTo+time';

      var range = _this.selector;
      var type = _EventModel.EventModel;
      database.select({
        store: store,
        index: 'replyTo+time',
        ...
        var eventKey = [record.content.sycamore.replyTo, record.received];

        if (!_this.selector.includes(eventKey)) {
          return;
        }

        matrix.getUserProfile(record.sender).then(function (profile) {
          var getMedia = matrix.getMedia(profile.avatar_url);
          return getMedia;
          ...
          _this.args.comments.push(comment);
        });
      });
      database.select({
        store: store,
        ...
        if (_this.likers.has(record.sender)) {
          return;
        }
        ...
        if (record.content.sycamore.reactTo === messageId) {
          _this.likers.add(record.sender);
      ...
      database.addEventListener('write', function (dbEvent) {
        ...
        matrix.getUserProfile(dbEvent.detail.record.sender).then(function (profile) {
          if (dbEvent.detail.subType !== 'insert') {
            return;
          }
          ...
          console.log(profile);
          var eventKey = [event.content.sycamore.replyTo, event.received];
  ...
  _createClass(MessageView, [{
    ...
    key: "verifiedClass",
    value: function verifiedClass(flag) {
      if (flag === true) {
        return 'verified';
      } else if (flag === false) {
    ...
    value: function createComment(event) {
      ...
      _Sycamore.Sycamore.getSettings().then(function (settings) {
        var matrixEvent = _this2.args.event;
        var message = {
          msgtype: 'm.text',
          body: _this2.args.commentInput,
          sycamore: {
            profile: 'https://sycamore.seanmorr.is/',
            "private": settings.privateFeed,
            "public": settings.publicFeed,
            replyTo: _this2.args.eventId
          }
        };
        var roomId = matrixEvent.content.sycamore["public"];
        return matrix.putEvent(roomId, 'm.room.message', message);
      }).then(function () {
        _this2.args.commentInput = '';
      });
    }
  }, {
    key: "createLike",
    value: function createLike(event) {
      ...
      _Sycamore.Sycamore.getSettings().then(function (settings) {
        var matrixEvent = _this3.args.event;
        var message = {
          msgtype: 'm.text',
          body: '👍',
          sycamore: {
            ...
            "private": settings.privateFeed,
            "public": settings.publicFeed,
            reactTo: _this3.args.event.event_id
          }
        };
        var roomId = matrixEvent.content.sycamore["public"];
        return matrix.putEvent(roomId, 'm.room.message', message);
      }).then(function () {
        _this3.args.commentInput = '';
      });
    }
  }, {
    key: "toggleComments",
...
;require.register("MessageYoutubeView.js", function(exports, require, module) {
"use strict";
...
exports.MessageYoutubeView = void 0;
...
var MessageYoutubeView = /*#__PURE__*/function (_MessageView) {
  _inherits(MessageYoutubeView, _MessageView);

  var _super = _createSuper(MessageYoutubeView);

  function MessageYoutubeView() {
    var _this;

    _classCallCheck(this, MessageYoutubeView);
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./message-youtube.html'));

    return _this;
  }

  return MessageYoutubeView;
}(_MessageView2.MessageView);

exports.MessageYoutubeView = MessageYoutubeView;
});
...
;require.register("Sycamore.js", function(exports, require, module) {
"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.Sycamore = void 0;
...
var Sycamore = /*#__PURE__*/function () {
  function Sycamore() {
    _classCallCheck(this, Sycamore);
  }

  _createClass(Sycamore, null, [{
    key: "getSettings",
    value: function getSettings(userId) {
      var _this = this;

      return matrix.getUserData('sycamore.settings').then(function (response) {
        if (!response) {
          return {};
        }
    ...
    key: "checkFeeds",
    value: function checkFeeds(userId) {
      this.getSettings(userId).then(function (settings) {
        var createPublic = Promise.resolve({
          room_id: settings.publicFeed
        });
        var createPrivate = Promise.resolve({
          room_id: settings.privateFeed
        });
          ...
          createPublic = matrix.createRoom('sycamore public feed', '', 'private', [{
            ...
            content: {
              ...
              events: (_events = {
                ...
                'm.room.message': 100
              }, _defineProperty(_events, "m.room.message", 100), _defineProperty(_events, 'm.room.avatar', 100), _defineProperty(_events, 'm.room.name', 100), _events),
              'ban': 100,
              'invite': 0,
              'kick': 100,
              'redact': 100,
          ...
          createPrivate = matrix.createRoom('sycamore private feed', '', 'private', [{
            ...
            content: {
              ...
              events: (_events2 = {
                ...
                'm.room.tombstone': 100,
                'm.room.message': 100
              }, _defineProperty(_events2, "m.room.message", 0), _defineProperty(_events2, 'm.room.avatar', 100), _defineProperty(_events2, 'm.room.name', 100), _events2),
              'ban': 100,
              'invite': 0,
              'kick': 100,
              'redact': 100,
        ...
        return Promise.all([createPublic, createPrivate]).then(function (_ref) {
          var _ref2 = _slicedToArray(_ref, 2),
              publicFeed = _ref2[0],
              privateFeed = _ref2[1];
...
_defineProperty(Sycamore, "settings", {});
});
  ...
  function WarehouseConsole(args) {
    ...
    _this.listen(_this.stream, 'ServerEvent', function (event) {
      try {
        ...
        fetch('data:application/sycamore;base64,' + frame.payload).then(function (response) {
          return response.arrayBuffer();
        }).then(function (bytes) {
          return _MessageModel.MessageModel.fromBytes(bytes);
        }).then(function (message) {
          return feed.displayPost(message);
        });
      } catch (error) {
        console.log(error);
      }
...
var _SettingsView = require("./ui/SettingsView");
...
var routes = {
  ...
  'settings': _SettingsView.SettingsView,
  'my-feed': function myFeed() {
    _Sycamore.Sycamore.getSettings().then(function (settings) {
      _Router.Router.go("/feed/".concat(settings.privateFeed));
    });
  },
...
var getToken = null;
var isGuest = false;
matrix.addEventListener('login', function () {
...
if (_Router.Router.query.loginToken) {
  matrix.completeSso(_Router.Router.query.loginToken);
} else {
...
var getDatabase = _EventDatabase.EventDatabase.open('events', 1);

Promise.all([getDatabase, getToken]).then(function (_ref) {
  var _ref2 = _slicedToArray(_ref, 2),
      database = _ref2[0],
      access_token = _ref2[1];

  _Sycamore.Sycamore.checkFeeds(token.user_id);

  matrix.addEventListener('matrix-event', function (throwEvent) {
    var event = _EventModel.EventModel.from(throwEvent.detail);
  ...
  matrix.sync().then(function (res) {
    if (!res || !res.rooms || !res.rooms.join) {
      return;
    }

    Object.entries(res.rooms.join).forEach(function (_ref3) {
      var _ref4 = _slicedToArray(_ref3, 2),
          room = _ref4[0],
          state = _ref4[1];
      ...
      state.timeline.events.forEach(function (chunk) {
        chunk.room_id = room;

        var event = _EventModel.EventModel.from(chunk);
      ...
      matrix.syncRoomHistory(room, state.timeline.prev_batch, function (chunk) {// const event = MatrixEvent.from(chunk);
        // const store = 'events';
...
require.register("matrix/EventDatabase.js", function(exports, require, module) {
"use strict";
...
var EventDatabase = /*#__PURE__*/function (_Database) {
  _inherits(EventDatabase, _Database);

  var _super = _createSuper(EventDatabase);
  ...
  _createClass(EventDatabase, [{
    key: "_version_1",
    value: function _version_1(database) {
      var eventStore = this.createObjectStore('events', {
        keyPath: 'event_id'
      });
      eventStore.createIndex('event_id', 'event_id', {
        unique: true
      });
      eventStore.createIndex('type', 'type', {
        unique: false
      });
      eventStore.createIndex('room_id', 'room_id', {
        unique: false
      });
      eventStore.createIndex('received', 'received', {
        unique: false
      });
      eventStore.createIndex('replyTo', ['content.sycamore.replyTo'], {
        unique: false
      });
      eventStore.createIndex('replyTo+time', ['content.sycamore.replyTo', 'received'], {
        unique: false
      });
      eventStore.createIndex('reactTo', ['content.sycamore.reactTo'], {
        unique: false
      });
      eventStore.createIndex('reactTo+time', ['content.sycamore.reactTo', 'received'], {
        unique: false
      });
      eventStore.createIndex('type+time', ['type', 'received'], {
        unique: false
      });
      eventStore.createIndex('room_id+time', ['room_id', 'received'], {
        unique: false
      });
      eventStore.createIndex('room_id+type+time', ['room_id', 'type', 'received'], {
        unique: false
      });
    }
  }]);
...
;require.register("matrix/EventModel.js", function(exports, require, module) {
"use strict";
...
var _EventDatabase = require("./EventDatabase");
...
var EventModel = /*#__PURE__*/function (_Model) {
  _inherits(EventModel, _Model);
  ...
  function EventModel() {
    ...
    _defineProperty(_assertThisInitialized(_this), "event_id", void 0);

    _defineProperty(_assertThisInitialized(_this), "type", void 0);

    _defineProperty(_assertThisInitialized(_this), "room_id", void 0);

    _defineProperty(_assertThisInitialized(_this), "content", void 0);

    _defineProperty(_assertThisInitialized(_this), "received", void 0);

    _defineProperty(_assertThisInitialized(_this), "origin_server_ts", void 0);
  ...
  _createClass(EventModel, [{
    ...
    value: function consume() {
      ...
      (_get2 = _get(_getPrototypeOf(EventModel.prototype), "consume", this)).call.apply(_get2, [this].concat(args));
...
;require.register("message-audio.html", function(exports, require, module) {
module.exports = "<li class = \"[[verified|verifiedClass]]\" data-type = \"[[header.mime]]\">\n\t<section class = \"author\">\n\t\t<div class = \"avatar\"></div>\n\t\t<span class = \"author\">\n\t\t\t<a cv-link = \"user/[[uid]]\">[[header.author]]</a>\n\t\t\t<div class = \"verify icon\"></div>\n\t\t</span>\n\t</section>\n\n\t<section>\n\t\t<small title = \"[[issued]]\">[[issued|formatTime]]</small>\n\t</section>\n\n\t<section>\n\t\t<span class = \"body video\">\n\t\t\t[[body]]\n\t\t\t<audio controls cv-attr = \"src:url\"></audio>\n\t\t</span>\n\t</section>\n\n\t<section class = \"reaction-bar\">\n\t\t<a cv-link>Reply</a>\n\t\t - <a cv-link>Like</a>\n\t\t - <a cv-link>Pay</a>\n\t\t - <a cv-link>Follow</a>\n\t</section>\n\n\t<section>\n\t\t<a cv-link = \"/messages/[[name]]\">\n\t\t\t[[name]]\n\t\t\t<img class = \"icon\" src = \"/go.svg\" />\n\t\t</a>\n\t</section>\n</li>\n"
});

;require.register("message-image.html", function(exports, require, module) {
module.exports = "<li class = \"[[verified|verifiedClass]]\" data-type = \"[[header.mime]]\">\n\t<section class = \"author\">\n\t\t<div class = \"avatar\"></div>\n\t\t<span class = \"author\">\n\t\t\t<a cv-link = \"user/[[uid]]\">[[header.author]]</a>\n\t\t\t<div class = \"verify icon\"></div>\n\t\t</span>\n\t</section>\n\n\t<section>\n\t\t<small title = \"[[issued]]\">[[issued|formatTime]]</small>\n\t</section>\n\n\t<section>\n\t\t<span class = \"body image\">\n\t\t\t<img cv-attr = \"src:url\">\n\t\t</span>\n\t</section>\n\n\t<section class = \"reaction-bar\">\n\t\t<a cv-link>Reply</a>\n\t\t - <a cv-link>Like</a>\n\t\t - <a cv-link>Pay</a>\n\t\t - <a cv-link>Follow</a>\n\t</section>\n\n\t<section>\n\t\t<a cv-link = \"/messages/[[name]]\">\n\t\t\t[[name]]\n\t\t\t<img class = \"icon\" src = \"/go.svg\" />\n\t\t</a>\n\t</section>\n</li>\n"
});

;require.register("message-link.html", function(exports, require, module) {
module.exports = "<li class = \"[[verified|verifiedClass]]\" data-type = \"[[header.mime]]\">\n\t<section class = \"author\">\n\t\t<div class = \"avatar\"></div>\n\t\t<span class = \"author\">\n\t\t\t<a cv-link = \"user/[[uid]]\">[[header.author]]</a>\n\t\t\t<div class = \"verify icon\"></div>\n\t\t</span>\n\t</section>\n\n\t<section>\n\t\t<small title = \"[[issued]]\">[[issued|formatTime]]</small>\n\t</section>\n\n\t<section>\n\t\t<span class = \"body text short\"><a href = \"[[body]]\">[[body]]</a></span>\n\t</section>\n\n\t<section class = \"reaction-bar\">\n\t\t<a cv-link>Reply</a>\n\t\t - <a cv-link>Like</a>\n\t\t - <a cv-link>Pay</a>\n\t\t - <a cv-link>Follow</a>\n\t</section>\n\n\t<section>\n\t\t<a cv-link = \"/messages/[[name]]\">\n\t\t\t[[name]]\n\t\t\t<img class = \"icon\" src = \"/go.svg\" />\n\t\t</a>\n\t</section>\n</li>\n"
});

;require.register("message-video.html", function(exports, require, module) {
module.exports = "<li class = \"[[verified|verifiedClass]]\" data-type = \"[[header.mime]]\">\n\t<section class = \"author\">\n\t\t<div class = \"avatar\"></div>\n\t\t<span class = \"author\">\n\t\t\t<a cv-link = \"user/[[uid]]\">[[header.author]]</a>\n\t\t\t<div class = \"verify icon\"></div>\n\t\t</span>\n\t</section>\n\n\t<section>\n\t\t<small title = \"[[issued]]\">[[issued|formatTime]]</small>\n\t</section>\n\n\t<section>\n\t\t<span class = \"body video\">\n\t\t\t[[body]]\n\t\t\t<video controls mute autoplay loop cv-attr = \"src:url\"></video>\n\t\t</span>\n\t</section>\n\n\t<section class = \"reaction-bar\">\n\t\t<a cv-link>Reply</a>\n\t\t - <a cv-link>Like</a>\n\t\t - <a cv-link>Pay</a>\n\t\t - <a cv-link>Follow</a>\n\t</section>\n\n\t<section>\n\t\t<a cv-link = \"/messages/[[name]]\">\n\t\t\t[[name]]\n\t\t\t<img class = \"icon\" src = \"/go.svg\" />\n\t\t</a>\n\t</section>\n</li>\n"
});

;require.register("message-youtube.html", function(exports, require, module) {
...
var SettingsView = /*#__PURE__*/function (_View) {
  ...
  function SettingsView() {
    ...
    _defineProperty(_assertThisInitialized(_this), "template", require('./settings.html'));

    _this.userId = matrix.getCurrentUserId();
    var editorDefaults = {
    ...
    var editor = new _codemirror["default"](function () {}, editorDefaults);

    _this.args.bindTo('content', function (v, k) {
      v = String(v);
    ...
    editor.on('change', function (editor, change) {
      _this.args.content = editor.getValue();
    });
    _this.args.editor = new _Tag.Tag(editor.getWrapperElement());
    _this.editor = editor;
    matrix.getUserData('sycamore.settings').then(function (response) {
      return _this.args.content = JSON.stringify(response || '{}', null, 2);
    })["catch"](function (error) {
      console.log(error);
      _this.args.content = '{}';
    });
  ...
  _createClass(SettingsView, [{
    ...
    value: function save(event) {
      ...
      var args = ['sycamore.settings', this.args.content];
      this.args.success = '';
      this.args.error = '';

      (_matrix = matrix).putUserData.apply(_matrix, args).then(function (response) {
        return _this2.args.success = 'Saved.';
      })["catch"](function (error) {
        return error.response.json();
      }).then(function (error) {
        return _this2.args.error = error.error;
      });
    }
  }]);
...
module.exports = "<section class = \"motif-page motif-box motif-rule page-settings\">\n\n\t<h1>Settings</h1>\n\n\t<section class = \"motif-alerts\">\n\t\t<section class = \"motif-alert motif-alert-success\">[[success]]</section>\n\t\t<section class = \"motif-alert motif-alert-error\">[[error]]</section>\n\t</section>\n\n\t<!-- <textarea cv-ref = \"textarea\" cv-bind = \"content\"></textarea> -->\n\n\t[[editor]]\n\n\n\t<section class=\"motif-buttons\">\n\t\t<button class=\"motif-button\" cv-on = \"click:save(event)\">save</button>\n\t</section>\n\n</section>\n"
});
...
;require.alias("codemirror/lib/codemirror.js", "codemirror");require.register("___globals___", function(exports, require, module) {
